...
public class MaxScoreForAgeGroups extends Configured implements Tool
{
...
// Tool runner will call run method automatically	
	 ...
	 */
	Job job = new Job();
	job.setJarByClass(MaxScoreForAgeGroups.class);
	...
	job.setMapperClass(MaxScoreForAgeGroupsMapper.class);
	job.setPartitionerClass(MaxScoreForAgeGroupsPartitioner.class);
	job.setCombinerClass(MaxScoreForAgeGroupsReducer.class);
	job.setReducerClass(MaxScoreForAgeGroupsReducer.class);
    ...
    {
    	int exitCode= ToolRunner.run(new Configuration(),new MaxScoreForAgeGroups(),args);

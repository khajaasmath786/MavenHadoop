...
public class ReadHDFSFileDriver extends Configured implements Tool
{
...
// Tool runner will call run method automatically	
	...
	if (args.length != 2)
	{
		 ...
		 // Input/ReadHDFSFile/Input.txt Output/ReadHDFSFile
	    System.exit(-1);
	}
	...
	job.setJarByClass(ReadHDFSFileDriver.class);
	...
	job.setCombinerClass(ReadHDFSFileReducer.class);
	job.setReducerClass(ReadHDFSFileReducer.class);
    ...
    {
    	int exitCode= ToolRunner.run(new Configuration(),new ReadHDFSFileDriver(),args);

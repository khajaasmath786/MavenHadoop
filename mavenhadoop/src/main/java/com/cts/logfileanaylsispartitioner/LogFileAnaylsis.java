...
public class LogFileAnaylsis extends Configured implements Tool
{
	...
	// Tool runner will call run method automatically	
		...
		if (args.length != 2)
		{
			 ...
			 //Input/logfileanaylsis Output/logfileanaylsisPartitioner
		    System.exit(-1);
		}
		 ...
		 */
		Job job = new Job();
		job.setJarByClass(LogFileAnaylsis.class);
		job.setJobName("logfileanaylsis");
		...
		job.setMapperClass(LogFileAnaylsisMapper.class);
		job.setNumReduceTasks(12);
		//job.setCombinerClass(LogFileAnaylsisReducer.class);
		job.setPartitionerClass(LogFileAnaylsisPartitioner.class);
		
		job.setReducerClass(LogFileAnaylsisReducer.class);
	    ...
	    {
	    	int exitCode= ToolRunner.run(new Configuration(),new LogFileAnaylsis(),args);

...
public class WordCountMapper extends Mapper<LongWritable, Text, Text, IntWritable>
{
	...
	/*
	 * Gets the parameters passed in configuration object
	 * */
	public void setup(Context cont)
	{
		Configuration conf=cont.getConfiguration();
		conf.set("country", "USA");
		conf.setInt("ZipCode", 19810);;
	}
	
	public void cleanup(Context cont)
	{
		Configuration conf=cont.getConfiguration();
		//conf.clear();
		conf.set("ZipCode", "1");
		int zip=conf.getInt("ZipCode", 0);
		System.out.println("Zip in destroy-->"+zip);
	}
    /*
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
    {
		...
		int zip=conf.getInt("ZipCode", 0);
		System.out.println("country---->"+country +"  Zip-->"+zip);
		System.out.println("columns for new map reduce program"+conf.get("test"));

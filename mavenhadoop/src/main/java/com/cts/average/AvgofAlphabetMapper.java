...
public class AvgofAlphabetMapper extends Mapper<LongWritable, Text, Text, IntWritable>
{
	...
	for (String word : line.split("\\W+"))
	{
	    if (word.length() > 0)
	    {
          String alphabet=word.substring(0, 1);
          System.out.println("alphabet"+alphabet);
          int lengthOfAlphabet=word.length();
		/*
		 * Call the write method on the Context object to emit a key and a value from the map method.
		 */
		context.write(new Text(alphabet), new IntWritable(lengthOfAlphabet));
	    }
	}
    }
}